...
credentials_path = '/mnt/c/Users/DATA/Desktop/script/tnmt-hoabinh-b0c8850da69a.json'

# Thư mục lưu file pdf cần check và share cho user "tnmt-hoabinh@tnmt-hoabinh.iam.gserviceaccount.com"
#folder_path = '/mnt/g/Shared drives/TNMT_HoaBinh3/3.BM.TK_001.07.05.H28_2010.1/'
folder_path = '/mnt/g/Shared drives/TNMT_HoaBinh3/3.BAN_GIAO/1.TAN_THINH/1.CGCN/Check'
...
sheet_name = "TanThinh-VanBan"


# Link file Exel cần ghi dữ liệu
file_path = "/mnt/c/Users/DATA/Desktop/script/Excel-CGCN.xlsx"
sheet_name = "CGCN"

# Link file txt cần ghi dữ liệu
file_path_txt = "/mnt/c/Users/DATA/Desktop/script/Excel-CGCN.txt"
...
def insert_data_to_excel(file_path, sheet_name, data):
    try:
        # Mở tệp Excel
    ...
    try:
        # Kiểm tra nếu sheet name không tồn tại, tạo sheet mới
        ...
        # Xóa dữ liệu cũ trong sheet (trừ tiêu đề cột) nếu có
        if sheet.max_row > 1:
            sheet.delete_rows(2, sheet.max_row)

        # Ghi dữ liệu mới vào sheet từ hàng thứ 2
        for row in data:
            sheet.append(row)
        ...
        print(f"Ghi đè dữ liệu thành công vào tệp Excel: {file_path} >> {sheet_name}")
    except Exception as e:
        print("Lỗi khi ghi đè dữ liệu vào tệp Excel:", str(e))



def insert_data_to_text_file(file_path_txt, data):
    try:
        # Mở file văn bản
        with open(file_path_txt, 'a+') as file:
            # Chuyển đổi các phần tử số nguyên thành chuỗi
            row_data_str = [str(item) for item in data]
            # Nối các giá trị cột thành chuỗi và ghi vào file
            line = ';'.join(row_data_str)
            file.write(line + '\n')

        print(f"Ghi dữ liệu thành công vào file: {file_path_txt}")
    except Exception as e:
        print("Lỗi khi ghi dữ liệu vào file:", str(e))
...
try:
    ...
    if len(pdf_files) > 0:
        ...
        # insert_data_to_excel(file_path, sheet_name, header)
        insert_data_to_text_file(file_path_txt, header)
        ...
        for pdf_file in pdf_files:
            try:
                ...
                if len(row_data) > 0:
                    ...
                    # insert_data_to_excel(file_path, sheet_name, row_data)
                    insert_data_to_text_file(file_path_txt, row_data)
        ...
        print(f"Kết quả đã được ghi vào file kết quả")
        print("##################################################################")
